...
const create = (user, callback) => {
    const queryString = "INSERT INTO jsusers (nume, prenume, cnp, email, datanastere, telefon, poza) VALUES (?, ?, ?, ?, ?, ?,?)";
    console.log(user);
    db_1.db.query(queryString, [user.nume, user.prenume, user.cnp, user.email, user.datanastere, user.telefon, user.poza], (err, result) => {
        if (err) {
            callback(err);
        }
...
const update = (user, callback) => {
    const queryString = `UPDATE jsusers SET nume=?, prenume=?, cnp=?, telefon=?, email=? WHERE id=?`;
    db_1.db.query(queryString, [user.nume, user.prenume, user.cnp, user.telefon, user.email, user.id], (err, result) => {
        if (err) {
            callback(err);
        }
        callback(null);
