...
export const create = (user: User, callback: Function) => {
  const queryString =
    "INSERT INTO jsusers (nume, prenume, cnp, email, datanastere, telefon, poza) VALUES (?, ?, ?, ?, ?, ?,?)";
    console.log(user);
  db.query(
    queryString,
    [user.nume, user.prenume, user.cnp, user.email, user.datanastere, user.telefon, user.poza],
    (err, result) => {
      if (err) {
        callback(err);
      }
...
export const update = (user: User, callback: Function) => {
  ...
  db.query(queryString, [user.nume, user.prenume, user.cnp, user.telefon, user.email, user.id], (err, result) => {
    if (err) {
      callback(err);
    }
    callback(null);
